...
export default function App() {
  ...
  useEffect(() => {
    const initDiagnosticApp = async () => {
      ...
      if (configured) {
        console.log(
          '🚀 [DIAGNOSTIC] FIREBASE MODE IS ACTIVE (isFirebaseConfigured() = true)'
        );
      } else {
        console.log(
          'ℹ️ [DIAGNOSTIC] LOCALSTORAGE FALLBACK MODE ACTIVE (isFirebaseConfigured() = false)'
        );
      }
      ...
      const unsubscribe = onAuthStateChanged(
        ...
        async (firebaseUser) => {
          try {
            if (firebaseUser) {
              console.log(
                '✅ [FIREBASE AUTH SESSION] Logged in user:',
                firebaseUser.uid,
                `(${firebaseUser.email})`
              );

              // Explicit check to verify if Firebase session token is available
              const token = await firebaseUser.getIdToken().catch((tokenErr) => {
                console.warn(
                  '⚠️ [AUTH TOKEN] Error fetching token in onAuthStateChanged:',
                  tokenErr
                );
                return null;
              });
              ...
              if (token) {
                console.log(
                  '✅ [AUTH TOKEN VERIFIED] Session token verified for:',
                  firebaseUser.uid
                );
              }
              ...
              console.log(
                'ℹ️ [FIREBASE SESSION] No active user session logged in.'
              );
            ...
            console.error(
              '❌ [FIREBASE AUTH OBSERVER CALLBACK EXCEPTION]:',
              err
            );
  ...
  const handleAddStudent = async (
    newStudentData: Omit<Student, 'id' | 'createdAt'>
  ) => {
  ...
  const handleUpdatePacket = async (updatedPacket: MeetingPacket) => {
    ...
    setPackets(
      packets.map((p) =>
        p.id === updatedPacket.id ? updatedPacket : p
      )
    );
  ...
  const currentStudentPackets = packets.filter(
    (p) => p.studentId === selectedStudentId
  );

  const ptiData = activeStudent
    ? STATE_PTI_CENTERS[activeStudent.state]
    : STATE_PTI_CENTERS['CA'];
    ...
    <div className="min-h-screen bg-slate-100 font-sans text-slate-900 flex flex-col justify-between">
      <div>
        ...
        {activeTab === 'landing' && (
          <LandingPage
            ...
            onViewSamplePacket={() => {
              const sample =
                packets.find((p) => p.id === 'packet_sample_maya') ||
                packets[0];
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                <div>
                  <div className="font-bold text-slate-900 text-sm flex items-center gap-2">
                    ...
                    <span
                      className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${
                        ...
                        user.subscriptionStatus === 'on_trial' ||
                        user.planTier === 'subscriber'
                  ...
                  <div className="text-slate-500 text-[11px]">
                    {user.planTier === 'subscriber'
                      ? `Unlimited AI Packet Generations ${
                      ...
                      : `${user.generationsCount || 0} of ${
                          user.maxFreeGenerations || 1
                        } free 1-page packets used`}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-indigo-900 text-white rounded-2xl p-4 shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border border-indigo-700">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-slate-200">
                    You have an unsubmitted intake draft for{' '}
            ...
            <div className="bg-slate-900 text-white rounded-3xl p-5 sm:p-8 shadow-xl border border-slate-800 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative overflow-hidden">
              <div className="space-y-3 max-w-2xl z-10">
                ...
                <p className="text-xs sm:text-sm text-slate-300 leading-relaxed font-normal">
                  Preparing for an upcoming meeting for{' '}
                  <strong>{activeStudent?.name}</strong> (
                  {activeStudent?.grade} &bull;{' '}
                  {activeStudent?.schoolDistrict})? Turn meeting notices into
                  1-page tactical printouts or launch Live Meeting Mode.
                </p>
              ...
              {activeStudent && (
                <div className="bg-slate-800/90 border border-slate-700/80 rounded-2xl p-4 w-full lg:w-72 text-xs space-y-2 shrink-0 z-10 text-slate-300">
                  ...
                  <div>
                    <strong>Grade:</strong> {activeStudent.grade}
                  </div>

                  <div>
                    <strong>District:</strong>{' '}
                    {activeStudent.schoolDistrict}
                  </div>
              ...
              <div className="lg:col-span-8 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <h2 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-2">
                    ...
                    <span>
                      Meeting Prep Packets for {activeStudent?.name}
                    </span>
                  </h2>
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 text-center space-y-4 shadow-sm">
                    ...
                    <div className="space-y-1">
                      <h3 className="font-bold text-slate-900 text-sm">
                        No Packets Generated Yet for {activeStudent?.name}
                      </h3>

                      <p className="text-xs text-slate-500 max-w-md mx-auto">
                        Create a 1-page print ready prep sheet for your
                        upcoming annual review or Section 504 meeting.
                      </p>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                          ...
                          <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                            ...
                            <p className="text-slate-700 text-[11.5px] font-medium line-clamp-2">
                              {pkt.content.topPriorities[0]?.title ||
                                'Meeting Preparation'}
                            </p>
                          </div>
                          ...
                          <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                            ...
                            <p className="text-slate-600 text-[11px] line-clamp-2">
                              {pkt.content.keyQuestions[0]?.question ||
                                'Specific questions formulated for team.'}
                            </p>
                          </div>
              ...
              <div className="lg:col-span-4 space-y-6">
                ...
                {ptiData && (
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3 text-xs">
                    ...
                    <div className="pt-1">
                      ...
                      >
                        <span>
                          Contact {ptiData.stateCode} PTI Center Hotline
                        </span>
                ...
                <div className="bg-slate-900 text-slate-300 rounded-2xl p-5 shadow-md border border-slate-800 text-xs space-y-2">
                  ...
                  <p className="text-slate-200 leading-relaxed text-[11.5px]">
                    <strong>
                      Never sign agreement on the spot if you feel pressured.
                    </strong>{' '}
                    Under federal IDEA guidelines, parents are equal team
                    members and have the right to take proposed IEP documents
                    home for 24-48 hours to review with family or an advocate.
                  </p>
        ...
        {activeTab === 'state-rights' && (
          <StateRightsDatabase
            initialStateCode={activeStudent?.state || 'CA'}
          />
        )}
        ...
        onLogin={async (email, name, id) => {
          ...
          if (isFirebaseConfigured() && auth.currentUser) {
            try {
              ...
              if (token) {
                console.log(
                  '✅ [AUTH TOKEN VERIFIED] Session token active for user:',
              ...
              console.warn(
                '⚠️ [AUTH TOKEN CHECK] Could not retrieve session token:',
                tokenErr
              );
      ...
      {isLiveMeetingOpen && activeStudent && (
        <LiveMeetingMode
          student={activeStudent}
          packet={
            packets.find((p) => p.studentId === activeStudent.id) || null
          }
